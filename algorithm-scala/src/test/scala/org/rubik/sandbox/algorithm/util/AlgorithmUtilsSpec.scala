...
class AlgorithmUtilsSpec extends FlatSpec {
  ...
  "top N for word frequency" should "top N" in {
    ...
    assert(AlgorithmUtils.topNOfWordFrequency(words, 2) == Seq(("abc",3), ("测试",2)))
  }

  "max multiply value of natural numbers" should "max value" in {
    assert(AlgorithmUtils.maxMultiplyValueOfNaturalNumber(6) == Seq(1, 2, 3))
    assert(AlgorithmUtils.maxMultiplyValueOfNaturalNumber(1000) == Seq(198, 199, 200, 201, 202))
  }
  ...
  "combination number" should "max number" in {
    ...
    assert(AlgorithmUtils.maxNumber(numbers) == "998969186666603533312210" )
  }
}